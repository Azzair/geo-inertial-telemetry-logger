...
export function convertToCSV(records: any[], comment?: string): string {
  ...
  const firstAbsTime = records[0]?.absTime ?? Date.now();

  for (const r of records) {
    const absTimeStr = formatAbsoluteTime(r.absTime || Date.now());
    const elapsedSeconds = Math.max(0, ((r.absTime || Date.now()) - firstAbsTime) / 1000);
    const relTimeStr = elapsedSeconds.toFixed(3);
    const row = [
      absTimeStr,
      relTimeStr,
